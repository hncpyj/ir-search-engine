"""
scifact adapter — science domain.

Dataset: mteb/scifact
  - config="corpus",  split="corpus"  → _id, title, text  (5 183 docs)
  - config="queries", split="queries" → _id, text          (1 109 queries)
  - config="default", split="test"    → query-id, corpus-id, score  (qrels)

Note: BeIR/scifact uses deprecated loading scripts → broken.
      mteb/scifact is the canonical replacement.
"""
...
class ScifactAdapter(BaseAdapter):
